import 'package:flutter/material.dart';
import 'package:google_fonts/google_fonts.dart';

class AppFontStyles {
  static TextStyle s8w500(double ratio) =>
      GoogleFonts.poppins(fontWeight: FontWeight.w500, fontSize: 8 * ratio);

  static TextStyle s12w500(double ratio) =>
      GoogleFonts.poppins(fontWeight: FontWeight.w500, fontSize: 12 * ratio);

  static TextStyle s12w600(double ratio) =>
      GoogleFonts.poppins(fontWeight: FontWeight.w600, fontSize: 12 * ratio);

  static TextStyle s12w400(double ratio) =>
      GoogleFonts.poppins(fontWeight: FontWeight.w400, fontSize: 12 * ratio);

  static TextStyle s14w600(double ratio) =>
      GoogleFonts.poppins(fontWeight: FontWeight.w600, fontSize: 14 * ratio);

  static TextStyle s14w700(double ratio) =>
      GoogleFonts.poppins(fontWeight: FontWeight.w700, fontSize: 14 * ratio);

  static TextStyle s16w500(double ratio) =>
      GoogleFonts.poppins(fontWeight: FontWeight.w500, fontSize: 16 * ratio);

  static TextStyle s16w400(double ratio) =>
      GoogleFonts.poppins(fontWeight: FontWeight.w400, fontSize: 16 * ratio);

  static TextStyle s16w600(double ratio) =>
      GoogleFonts.poppins(fontWeight: FontWeight.w600, fontSize: 16 * ratio);

  static TextStyle s18w600(double ratio) =>
      GoogleFonts.poppins(fontWeight: FontWeight.w600, fontSize: 18 * ratio);

  static TextStyle s24w600(double ratio) =>
      GoogleFonts.poppins(fontWeight: FontWeight.w600, fontSize: 24 * ratio);

  static TextStyle s26w600(double ratio) =>
      GoogleFonts.poppins(fontWeight: FontWeight.w600, fontSize: 26 * ratio);
}
